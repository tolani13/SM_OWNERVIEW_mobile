...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const handleLogout = () => {
    // For now, just reload to login page
    // Later this will clear auth tokens and redirect
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t z-50 flex justify-around p-2 pb-4 safe-area-bottom">
        {NAV_ITEMS.map((item) => {
           const isActive = location === item.href || (item.href !== "/" && location.startsWith(item.href));
           return (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center p-2 rounded-lg", isActive ? "text-primary" : "text-muted-foreground")}>
